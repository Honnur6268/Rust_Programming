...
#[post("/login")]
async fn authenticate(client: web::Data<Database>, login_request: web::Json<LoginRequest>) -> HttpResponse {
    if let Err(validation_errors) = login_request.validate() {
        return handle_validation_errors(&validation_errors);
    }
    match user_service::authenticate(&client, login_request.into_inner()).await {
